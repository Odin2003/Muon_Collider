...
# Input LCIO file for 1000 GeV
#file_path = "/lustre/work/odschnei/USMCC/mu_mu_ax_gamma/m1000_3TeV_20GeV_gamma/reco/output_reco_run100.slcio"
file_path = "../reco/output_reco.slcio"
output_file = "histograms_m20.root"
...
histograms = {
        "dR_bs": ROOT.TH1F("dR_bs_1000", "Delta R (b, bbar) (1000 GeV); dR; Counts", 50, 0, 4),
        "girth": ROOT.TH1F("girth_1000", "Jet Girth (1000 GeV); Girth; Counts", 50, 0, 1),
        "jet_mass": ROOT.TH1F("jet_mass_1000", "Jet Mass (1000 GeV); Mass [GeV]; Counts", 15000, 0, 3000),
        "jet_pt": ROOT.TH1F("jet_pt_1000", "Jet pT (1000 GeV ALP); pT [GeV]; Counts", 15000, 0, 3000),
        }
...
R = 0.8         # Change to 0.4 for resolved analysis
jet_def = fj.JetDefinition(fj.antikt_algorithm, R)

for event in reader:
    try:
        signal.alarm(15 * 60)  # Set the alarm for 15 minutes to avoid getting stuck on some events
        print(f"Processing Event: {event.getEventNumber()}")

        collection = event.getCollection("MCParticle")      # This is the truth level collection that we use to look at b quarks
        particles = []
        ...
        for mc in collection:       # Getting b quarks and their four-momentum
        ...
        if b_quark and bbar_quark:
            dR_bs = delta_r(b_quark, bbar_quark)        # seeing how far the quarks are apart
            histograms["dR_bs"].Fill(dR_bs)
        else:
            print(f"Event {event.getEventNumber()}: Missing b or bbar quark")

        collection = event.getCollection("PandoraPFOs")     # These are reconstructed particle flow objects used for real analysis
        particles = []
        ...
        sequence = fj.ClusterSequence(particles, jet_def)       # applying fasjet to cluster jets
        ...
        for jet in jets:
            ...
            if jet_mass > 2.0:
                ...
                print("photon jet mass: ", jet_mass)
